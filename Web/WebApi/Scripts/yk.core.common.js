...
(function (ns) {
    ...
    ns.post = function (url, paras, showId) {
        ...
        xmlHttpRequest.setRequestHeader("Content-Length", paras == null || paras == undefined ? 0 : paras.lenght);
    ...
    //加载选项卡
    ns.loadTab = function () {
        $(".tabTitle div").width(($(window).width() - ($(".tabTitle div").length - 1)) / $(".tabTitle div").length);
        $(".tabTitle div").each(function (index) {
            if (index == 0) {
            ...
            this.onclick = function () {
                ...
                $(".tabDiv").eq(index).css("display", "block");
                $(".tabTitle div").removeClass("tabOn");
                $(this).addClass("tabOn");
            }
        });
        //底部
        var bottomALength = $(".bottomButton a").length;
        $(".bottomButton a").width(($(window).width() - (bottomALength - 1)) / bottomALength);
        if (bottomALength > 0)
        {
    ...
    ns.locationReplace = function () {
        location.replace(location.href);
    }
    //ajax提交
    ns.ajaxSubmit = function (url,param,callback) {
        $.ajax({
            url: url,
            type: 'post',
            data: param,

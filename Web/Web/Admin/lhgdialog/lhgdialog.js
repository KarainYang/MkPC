/*!
 * lhgcore Dialog Plugin v3.4.2
 * Date : 2011-05-26 09:35:11
...
reSizeHdl = function() {
    ...
    J('head', doc).append('<link href="' + getPath + 'skins/' + dgSkin + '.css" rel="stylesheet" type="text/css"/>');
	...
	removeDG = function() {
	    ...
	    if (opt.autoPos) {
	        if (J.browser.msie && J.browser.version < 8)
	            top.document.body.onresize = null;
	        else
        ...
        S.ShowDialog = function() {
            ...
            if (opt.autoPos) {
                if (J.browser.msie && J.browser.version < 8)
                    top.document.body.onresize = dgAutoPos;
                else
                    J(top).bind('resize', dgAutoPos);
            }
        };
